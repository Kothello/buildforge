...
const Index = () => {
  ...
  const pickLeanToWall = (cam?: { x: number; y: number; z: number }): { leanToId: string; leanToWall: 'front' | 'back' | 'left' | 'right' } | null => {
    ...
    // For each lean-to, check ALL walls and pick the best match
    for (const leanTo of leanTos) {
      ...
      // Vector from lean-to center to camera
      ...
      // Front wall check (away from main building) - more lenient requirements
      ...
      // Side walls and back wall for enclosed/gable lean-tos
      if (leanTo.type === 'gable' || leanTo.type === 'enclosed') {
        // For lean-tos on left/right of main building
        if (leanTo.wall === 'right' || leanTo.wall === 'left') {
          // Left side wall (negative Z) - more lenient
          if (normalized.z < -0.5) {
            const score = Math.abs(normalized.z);
            if (score > 0.5 && (!bestMatch || score > bestMatch.score)) {
          ...
          // Right side wall (positive Z) - more lenient
          if (normalized.z > 0.5) {
            const score = normalized.z;
            if (score > 0.5 && (!bestMatch || score > bestMatch.score)) {
          ...
          // Back wall - only for gable (toward main building)
        ...
        // For lean-tos on front/back of main building
        if (leanTo.wall === 'front' || leanTo.wall === 'back') {
          // When lean-to is on front/back, it's rotated 90° so left/right are reversed from camera perspective
          // Right side wall (negative X from camera view) 
          if (normalized.x < -0.5) {
            const score = Math.abs(normalized.x);
            if (score > 0.5 && (!bestMatch || score > bestMatch.score)) {
              bestMatch = { leanToId: leanTo.id, leanToWall: 'right', score };
          ...
          // Left side wall (positive X from camera view)
          if (normalized.x > 0.5) {
            const score = normalized.x;
            if (score > 0.5 && (!bestMatch || score > bestMatch.score)) {
              bestMatch = { leanToId: leanTo.id, leanToWall: 'left', score };
  ...
  const getAxisLength = (wall: Door['wall'], leanToId?: string, leanToWall?: 'front' | 'back' | 'left' | 'right') => {
    if (leanToId && leanToWall) {
      ...
      if (leanTo) {
        const effectiveWidth = leanTo.type === 'gable' ? leanTo.length : leanTo.width;
        const effectiveLength = leanTo.type === 'gable' ? leanTo.width : leanTo.length;
        // Match 3D placement logic:
        // - For gable: front/back use effectiveWidth, left/right use effectiveLength
        // - For single-slope: front/back run along lean-to length, ends use lean-to width
        if (leanTo.type === 'gable') {
  ...
  const clampToBounds = (wall: Door['wall'], pos: number, doorWidthFt: number, leanToId?: string, leanToWall?: 'front' | 'back' | 'left' | 'right') => {
    ...
    if (leanToId && leanToWall) {
      ...
      if (leanTo) {
        if (leanTo.type !== 'gable') {
          // Single-slope: use reduced gap for all walls
          ...
          // Gable: different gap for different walls
          if (leanToWall === 'left' || leanToWall === 'right') {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'hsl(var(--background))' }}>
      {/* Header - sticky */}
      <header className="sticky top-0 z-20 border-b m-0 p-0" style={{ borderColor: 'hsl(var(--border))', background: 'hsl(var(--background))' }}>
      ...
      <div className="flex flex-col md:flex-row lg:flex-row m-0 p-0 md:h-[calc(100vh-49px)] lg:h-[calc(100vh-49px)]">
        {/* 3D Viewer - sticky on mobile, fixed size on desktop */}
        <div className="sticky md:static lg:static z-10 h-[27.5vh] md:h-full lg:h-full w-full md:w-[62%] lg:w-[62%] px-3 py-2" style={{ top: '49px', background: 'hsl(var(--background))' }}>
